...
export default function GridBlurredBackdrop() {
  const [dataShuffle, updateData] = useState(shuffle(testimonials));
  const toast = useToast();
  const onClick = () => {
    toast({
      title: 'Data Refreshed.',
      position: 'top',
      description: 'Updated the Alumni Advice.',
      status: 'success',
      duration: 9000,
      isClosable: true,
    });
    updateData(shuffle(testimonials).slice());
  };
  ...
  return (
    ...
    >
      <Box width={{ base: 'full', sm: 'lg', lg: 'xl' }} margin={'auto'}>
        <Container>
          <Heading>CUNY Tech Prep Advice</Heading>
        </Container>
      </Box>
      <Box>
        <Center>
          <SimpleGrid columns={{ sm: 1, xl: 2 }} spacing={30} mt={10}>
